...
ax4 = plt.subplot2grid(shape, (1,0), colspan=colspan, rowspan=rowspan)
ax5 = plt.subplot2grid(shape, (1,2), colspan=colspan, rowspan=rowspan)
ax6 = plt.subplot2grid(shape, (1,4), colspan=colspan, rowspan=rowspan)

axes = [ax1, ax2, ax3, ax4, ax5, ax6]

list_of_env = ['Hopper-v3', 'Walker2d-v3', 'HalfCheetah-v3', 'Ant-v3', 'Humanoid-v3', 'HumanoidStandup-v2']


x_new = np.arange(0, 1e6, 10000)
x_web = np.arange(0, 1e6, 5000)
for i, env in enumerate(list_of_env):
	etd_result = parse_multiple_seed(f"CEM_SAC/{env}", type='pkl')
	cem_result = parse_multiple_seed(f"CEM/{env}", type='pkl')
	cemtd_result = parse_multiple_seed(f"CEM_TD3/{env}", type='pkl')
	baseline_result = parse_multiple_seed(f"SAC/{env}", interpolated_length=5000, type='csv')

	if args.compute:
		print('\nenv:', env)
		print('CEM_SAC', round(np.median(etd_result[:,-1]),3), '&', round(np.mean(etd_result[:,-1]),3), '&',round(np.std(etd_result[:,-1]),3))
		print('CEM-TD3', round(np.median(cemtd_result[:,-1]),3), '&', round(np.mean(cemtd_result[:,-1]),3), '&',round(np.std(cemtd_result[:,-1]),3))
		print(f'SAC', round(np.median(baseline_result[:,-1]),3), '&', round(np.mean(baseline_result[:,-1]),3), '&',round(np.std(baseline_result[:,-1]),3))
	...
	if args.value_type == 'average':
		...
		cem = np.mean(cem_result, axis=0)
		cem_std = np.std(cem_result, axis=0)
		baseline = np.mean(baseline_result, axis=0)
		baseline_std = np.std(baseline_result, axis=0)
		axes[i].fill_between(x_new, etd + etd_std, etd - etd_std, alpha=0.3, color='magenta')
		axes[i].fill_between(x_new, cemtd + cemtd_std, cemtd - cemtd_std, alpha=0.2, color=COLORS[2])
		axes[i].fill_between(x_new, cem + cem_std, cem - cem_std, alpha=0.2, color='cyan')
		axes[i].fill_between(x_web, baseline + baseline_std, baseline - baseline_std, alpha=0.2, color=COLORS[1])

	elif args.value_type == 'median':
		etd = np.median(etd_result, axis=0)
		q75, q25 = np.percentile(etd_result, [90 ,10], axis=0)
		axes[i].fill_between(x_new, q75, q25, alpha=0.3, color='magenta')

		cemtd = np.median(cemtd_result, axis=0)
		q75, q25 = np.percentile(cemtd_result, [90 ,10], axis=0)
		axes[i].fill_between(x_new, q75, q25, alpha=0.2, color=COLORS[2])
		
		baseline = np.median(baseline_result, axis=0)
		q75, q25 = np.percentile(baseline_result, [90 ,10], axis=0)
		axes[i].fill_between(x_web, q75, q25, alpha=0.2, color=COLORS[1])

	axes[i].plot(x_new, etd, label='CEM-SAC', color='magenta')
	
	axes[i].plot(x_new, cemtd, label='CEM-TD3', color=COLORS[2])
	axes[i].plot(x_new, cem, label='CEM', color='blue', linestyle='dashed')
	axes[i].plot(x_web, baseline, label=f'SAC', color=COLORS[1])
	axes[i].legend(loc='lower right')
	axes[i].set_xlabel('Interaction steps')
	if i==0 or i ==3:
		axes[i].set_ylabel('Simulation score')
	axes[i].set_title(f'{env}')
plt.tight_layout()

if args.save_fig:
	plt.savefig(f'./cec_{args.value_type}.pdf')
else:
	plt.show()
